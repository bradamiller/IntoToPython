...
*If your course covers the Module 2 Optional Extension (classes), everything below works identically through `tracker.track_until_cross()`, `tracker.turn_right()`, and `tracker.turn_left()` instead of bare function calls.*
...
- **Clearing already happens inside turns**: `turn_right()`/`turn_left()` call `clear_intersection()` themselves — no extra clearing needed before or after a turn
...
3. **Review: `turn_right()` and `turn_left()`**:
   - Both functions from Module 2 Lesson 9:
     - Call `clear_intersection()` to drive past the intersection first
     - Start spinning in the desired direction
     - Stop when sensors find the perpendicular line
   - The turn clears the intersection automatically — no extra clearing needed after turning, and none needed before it either (the turn function does it internally)
...
3. **Using the `drive_intersections()` function from Lesson 2**:
   ...
   drive_intersections(2)  # Drive forward 2
...
## Common Issues
- **Robot turns but doesn't find the line**: The perpendicular line may be too far. Check grid spacing and the turn's clearing distance
- **Robot goes the wrong direction after turning**: Verify which turn function you used (right vs. left)
