...
**What makes numeric headings and modular arithmetic so powerful for solving the turning problem?** Using numbers 0-3 means we can calculate turns with a single formula: `(needed - current) % 4`. The formula works for all 16 combinations automatically. It's short to write, hard to get wrong, and reveals the underlying mathematical structure: headings are arranged in a circle, and the modulo operation naturally wraps around. This approach is also easy to extend -- if you wanted to add diagonal headings, you would just use 8 directions instead of 4 with the same formula pattern.
