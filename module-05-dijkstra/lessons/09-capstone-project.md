...
- Integrate Dijkstra pathfinding, driving functions, rangefinder obstacle detection, and obstacle memory into a single working program
...
- Organize code into well-named functions with clear comments and readable structure
- Debug integration issues where individually working components fail to work together

## Key Concepts
- **System integration**: Combining multiple independently developed components (Dijkstra functions, driving functions, rangefinder, file I/O) into a single program that works as a unified whole. Integration is often harder than building individual pieces because components must communicate correctly through shared data structures like the blocked list.
...
- **Path recomputation**: When an obstacle is detected during path execution, the robot stops, adds the obstacle to the blocked list, rebuilds the graph with `build_dijkstra_graph()`, and computes a fresh path from its current position to the destination. This process may happen multiple times per destination.
...
- Completed code from Lessons 4-8 (Dijkstra functions, driving functions, obstacle detection, file I/O)
...
1. **Hook: This Is Your Graduation Project!**
   - Display the journey table on the board:
     ...
     Lesson 6:  Testing and swapping between Manhattan and Dijkstra
...
2. **Project Requirements Overview**
   - Walk through the eight requirements:
     ...
     2. Use `compute_dijkstra_path()` to compute paths
   ...
   - Display the overall program structure on the board or projector:
     ...
     Step 3: Main navigation loop
       For each destination:
         While not arrived:
           Compute path with compute_dijkstra_path()
...
5. **Step 4: Connecting the Pieces**
   - "The key insight is data flow. The `blocked` list is the thread that connects everything:"
     ...
     - `build_dijkstra_graph` receives `blocked` and uses it to build the graph
...
| **Code Quality** | 5 | Code is readable, uses well-named functions, has meaningful comments |

## Common Misconceptions

| Misconception | Reality |
|---|---|
| "I need to write all the code from scratch for the capstone" | The capstone is an INTEGRATION project. You already have `compute_dijkstra_path()` (Lessons 4-5), obstacle detection logic (Lesson 7), and file I/O functions (Lesson 8). The work is connecting these pieces together with a main navigation loop. |
...
- Provide the capstone starter file which has all functions complete and only requires filling in the main loop logic
...
### Driving Function with DifferentialDrive
```python
# navigator_xrp.py
# Functions that translate grid paths into XRP robot movements

from XRPLib.differential_drive import DifferentialDrive

drivetrain = DifferentialDrive.get_default_differential_drive()
heading = "north"  # Robot starts facing north (toward row 0)
GRID_SPACING = 30  # cm between intersections

def drive_path_capstone(path):
    """Drive the robot along a list of (row, col) positions."""
    for i in range(len(path) - 1):
        current = path[i]
        next_pos = path[i + 1]
        drive_segment(current, next_pos)

def drive_segment(current, next_pos):
    ...
    turn_amount = get_turn_angle(heading, target_heading)
    if turn_amount != 0:
        drivetrain.turn(turn_amount)
    heading = target_heading

    # Drive forward one grid spacing
    drivetrain.straight(GRID_SPACING)

def get_turn_angle(current_heading, target_heading):
...
- **Expect integration bugs.** The most common issues are: (1) the blocked list not persisting between destinations because it gets re-created, (2) the robot's `current` position not being updated after each step, (3) the graph not being rebuilt with the updated blocked list after an obstacle is found, and (4) the file I/O functions writing or reading in the wrong format.
- **Use the rubric as a checklist.** Have students self-assess against the rubric before demo day. Each rubric category maps to a specific lesson: Dijkstra (Lessons 4-5), Driving Integration (Module 4 Lesson 8 / Lesson 6), Obstacle Detection (Lesson 7), Experience/File I/O (Lesson 8).
...
- **Module 2** built on those fundamentals with loops, functions, and more complex logic. Students gained the ability to write reusable code and organize it into toolkits.
- **Module 3** applied that toolkit to the grid, and started using tuples, lists, and separation of concerns as the codebase grew.
- **Module 4** introduced the Manhattan Distance algorithm and grid navigation. Students built their first pathfinding system with a clean, reusable function interface. (Courses covering classes also learned about methods and object-oriented design here.)
